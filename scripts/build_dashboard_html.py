"""
build_dashboard_html.py — Regenerate spending_dashboard.html data from CSVs.

The HTML template `spending_dashboard.html` ships with two marked regions in
its <script> block:

    // __DATA_START__
    const ALL_TXN = [ ... ];        ◄── regenerated from expenses.csv
    // __DATA_END__

    // __BALANCE_START__
    const BOA_BALANCE = [ ... ];    ◄── regenerated from bofa_balance.csv
    // __BALANCE_END__

This script reads both CSVs, formats every row into a JS object literal, and
rewrites the content between those markers. The HTML's styling, charts, and
Chart.js logic are untouched — only the underlying data is refreshed.

Pipeline shape (mirrors build_dashboard.py for Excel):

    expenses.csv     ─┐
                      ├─► build_dashboard_html.py ──► spending_dashboard.html
    bofa_balance.csv ─┘

Run:
    python scripts/build_dashboard_html.py

Idempotent — safe to re-run after every clean_data.py / extract_pdfs.py update.
If bofa_balance.csv is missing the transaction data is still refreshed and the
balance block is left as-is (with a warning).
"""
...
def build_balance_block(balance_rows: list[dict]) -> str:
    """Render the const BOA_BALANCE = [ ... ]; block from bofa_balance.csv rows.

    The CSV stores ISO dates (2025-12-24); the dashboard wants short labels
    (Dec 24), so each row is reformatted on the way in.
    """
    lines = [
        "// __BALANCE_START__  (regenerated by scripts/build_dashboard_html.py from bofa_balance.csv)",
        "const BOA_BALANCE = [",
    ]
    for row in balance_rows:
        label = datetime.strptime(row["Date"], "%Y-%m-%d").strftime("%b %d")
        val = float(row["Balance"])
        lines.append(f"  {{label:'{label}', val:{val:.2f}}},")
    lines.append("];")
    lines.append("// __BALANCE_END__")
    return "\n".join(lines)
...
def main() -> None:
    ...
    # --- Transaction data block (required) ---
    data_pattern = re.compile(r"// __DATA_START__.*?// __DATA_END__", re.DOTALL)
    ...
    html = data_pattern.sub(build_data_block(rows), html, count=1)

    # --- Checking-balance block (optional — skipped if CSV absent) ---
    balance_note = "  balance:     bofa_balance.csv not found — balance block left unchanged"
    if BALANCE_CSV.exists():
        with BALANCE_CSV.open(newline="", encoding="utf-8") as fh:
            balance_rows = list(csv.DictReader(fh))
        bal_pattern = re.compile(r"// __BALANCE_START__.*?// __BALANCE_END__", re.DOTALL)
        if not bal_pattern.search(html):
            raise SystemExit(
                "bofa_balance.csv exists but the // __BALANCE_START__ ... "
                "// __BALANCE_END__ markers are missing from spending_dashboard.html."
            )
        html = bal_pattern.sub(build_balance_block(balance_rows), html, count=1)
        latest = balance_rows[-1]
        balance_note = (
            f"  balance:     {len(balance_rows)} points, "
            f"latest ${float(latest['Balance']):,.2f} on {latest['Date']}"
        )
